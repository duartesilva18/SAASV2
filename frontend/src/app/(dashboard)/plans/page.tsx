...
export default function PlansPage() {
  ...
  return (
    <div className="space-y-6 3xl:space-y-16 3xl:md:space-y-20 pb-16 3xl:pb-20 px-3 sm:px-4 md:px-8 3xl:pt-10 pt-2 sm:pt-6 max-w-7xl mx-auto">
      ...
      <section className="text-center mb-4 3xl:mb-12 3xl:md:mb-20 3xl:lg:mb-28">
        <motion.h2
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="text-lg sm:text-xl md:text-2xl 3xl:text-5xl 3xl:lg:text-6xl font-black tracking-tighter mb-2 3xl:mb-6 uppercase leading-tight"
        >
          Quanto vale ter{' '}
        ...
        <motion.p
          ...
          className="text-xs sm:text-sm 3xl:text-base 3xl:md:text-lg 3xl:lg:text-2xl text-slate-400 mb-2 3xl:mb-6 3xl:md:mb-8 max-w-2xl mx-auto"
        >
        ...
        <motion.p
          ...
          className="text-xs sm:text-sm 3xl:text-base 3xl:md:text-lg 3xl:lg:text-2xl text-white font-semibold mb-4 3xl:mb-8 3xl:md:mb-10 max-w-2xl mx-auto"
        >
      ...
      <section>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 3xl:gap-6 3xl:lg:gap-8 items-stretch">
          {plans.map((plan: any, index: number) => (
            <motion.div
              ...
              className={`relative rounded-xl 3xl:rounded-3xl p-4 sm:p-5 3xl:p-6 3xl:md:p-8 3xl:lg:p-9 overflow-visible group transition-all duration-300 flex flex-col ${
                plan.popular 
            ...
            >
              {isCurrentPlan(plan.priceId) && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className="absolute -top-2.5 3xl:-top-3 left-1/2 -translate-x-1/2 bg-gradient-to-r from-emerald-600 via-green-600 to-emerald-600 text-white px-3 py-1.5 3xl:px-6 3xl:py-2.5 rounded-xl 3xl:rounded-2xl text-xs 3xl:text-sm font-black uppercase tracking-[0.15em] 3xl:tracking-[0.2em] shadow-lg flex items-center gap-1.5 3xl:gap-2 z-30 whitespace-nowrap"
                >
              ...
              {plan.popular && plan.popularLabel && !isCurrentPlan(plan.priceId) && (
                <div className="absolute -top-2.5 3xl:-top-3 left-1/2 -translate-x-1/2 bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-600 text-white px-3 py-1.5 3xl:px-6 3xl:py-2.5 rounded-xl 3xl:rounded-2xl text-xs 3xl:text-sm font-black uppercase tracking-[0.15em] 3xl:tracking-[0.2em] shadow-lg flex items-center gap-1.5 3xl:gap-2 z-30 whitespace-nowrap">
              ...
              <div className="relative z-10 flex flex-col flex-1 min-h-0">
                <div className="flex-1 flex flex-col">
                  <div className="flex items-center justify-between mb-3 3xl:mb-5">
                    <div className={`w-12 h-12 3xl:w-16 3xl:h-16 rounded-xl 3xl:rounded-2xl flex items-center justify-center shrink-0 ${
                      plan.popular 
                        ? 'bg-blue-500/20 border-2 border-blue-500/40' 
                        : 'bg-slate-700/80 border border-slate-600/50'
                    }`}>
                      <plan.icon className="w-6 h-6 3xl:w-8 3xl:h-8" style={{ color: plan.popular ? '#60a5fa' : '#94a3b8' }} />
                    </div>
                    <div className="text-right min-w-0">
                      <p className="text-[10px] 3xl:text-sm font-black uppercase tracking-widest text-slate-400 mb-0.5 3xl:mb-1 truncate">{plan.name}</p>
                      <p className="text-2xl sm:text-3xl 3xl:text-4xl 3xl:md:text-5xl font-black text-white tracking-tighter leading-none">
                        {plan.price}
                      </p>
                      <p className="text-[10px] 3xl:text-sm text-slate-400 font-semibold mt-0.5">{plan.priceSuffix}</p>
                      {plan.priceSecondary && (
                        <p className="text-xs 3xl:text-base text-emerald-400 font-semibold mt-1 3xl:mt-1.5">{plan.priceSecondary}</p>
                      )}
                    </div>
                  </div>

                  <p className="text-xs 3xl:text-base 3xl:md:text-lg text-slate-400 mb-1 3xl:mb-2 font-medium line-clamp-2 3xl:line-clamp-none">{plan.tagline}</p>
                  <p className="text-xs 3xl:text-base text-slate-500 mb-4 3xl:mb-6 italic line-clamp-2 3xl:line-clamp-none">&quot;{plan.quote}&quot;</p>

                  <div className="space-y-2 3xl:space-y-3 mb-4 3xl:mb-6">
                    {plan.features.map((feature: string, fIndex: number) => (
                      <div key={fIndex} className="flex items-start gap-2 3xl:gap-3">
                        <Check size={18} className="text-emerald-400 mt-0.5 shrink-0 3xl:w-[22px] 3xl:h-[22px]" />
                        <p className="text-xs 3xl:text-base 3xl:md:text-lg text-slate-200 font-medium">{feature}</p>
                      </div>
                    ))}
                  </div>

                  {plan.limitation && (
                    <p className="text-xs 3xl:text-base text-amber-400/90 mb-4 3xl:mb-6 font-medium">🚫 {plan.limitation}</p>
                  )}
                </div>
                ...
                <button
                  ...
                  className={`mt-auto w-full block text-center px-4 3xl:px-6 py-3 3xl:py-4 rounded-xl 3xl:rounded-2xl text-sm 3xl:text-base font-black uppercase tracking-[0.15em] 3xl:tracking-[0.2em] transition-all cursor-pointer ${
                    isCurrentPlan(plan.priceId)
      ...
      <motion.section
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="max-w-3xl mx-auto text-center mt-6 3xl:mt-16 mb-6 3xl:mb-16"
      >
        <div className="inline-flex items-center gap-2 px-4 py-2 3xl:px-6 3xl:py-3 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs 3xl:text-base font-black uppercase tracking-[0.15em] 3xl:tracking-[0.2em] mb-4 3xl:mb-8">
          Programa de Afiliados FinLy
        </div>
        <h3 className="text-xl sm:text-2xl 3xl:text-4xl 3xl:md:text-5xl font-black text-white tracking-tight mb-3 3xl:mb-6 leading-tight">
          Usa. Indica. Ganha.
        </h3>
        <p className="text-slate-400 text-sm 3xl:text-lg 3xl:md:text-xl mb-4 3xl:mb-10">
          Sempre que alguém entra na FinLy pelo teu link:
        </p>
        <ul className="flex flex-col items-center text-slate-200 text-sm 3xl:text-lg 3xl:md:text-xl space-y-2 3xl:space-y-4 mb-6 3xl:mb-12 font-medium">
        ...
        <div className="bg-slate-800/90 border border-slate-600/60 rounded-2xl 3xl:rounded-3xl p-4 3xl:p-6 3xl:md:p-8 shadow-xl">
          <p className="text-[10px] 3xl:text-sm font-black uppercase tracking-widest text-slate-400 mb-2 3xl:mb-4">Exemplo simples:</p>
          <p className="text-sm 3xl:text-lg 3xl:md:text-xl text-slate-200 font-medium mb-1">10 pessoas no plano mensal → ~20€ por mês</p>
          <p className="text-sm 3xl:text-lg 3xl:md:text-xl text-slate-200 font-medium mb-3 3xl:mb-5">50 pessoas → ~100€ por mês</p>
          <p className="text-xs 3xl:text-base text-slate-500">Sem anúncios. Sem suporte. Sem esforço extra.</p>
        </div>
      </motion.section>
      ...
      <motion.section
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="pt-6 3xl:pt-16 border-t border-white/5"
      >
        <div className="text-center mb-3 3xl:mb-8">
          <span className="inline-flex items-center gap-2 text-slate-500 text-xs 3xl:text-sm font-black uppercase tracking-[0.15em] 3xl:tracking-[0.2em]">
            Sem risco, sem letras pequenas
          </span>
        </div>
        <div className="flex flex-wrap justify-center gap-4 3xl:gap-12">
          <div className="flex items-center gap-2 3xl:gap-3 text-slate-400 text-xs 3xl:text-base">
